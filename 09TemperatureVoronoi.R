...
temperature <- brick('~/Downloads/global-reanalysis-phy-001-031-grepv2-monthly_1612036498812.nc')

temperatureAv <- calc(temperature, fun = mean, na.rm = T)
myCrs <- crs('+proj=stere +lat_0=-90 +lon_0=-15 +k=1 +x_0=-15 +y_0=0 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0')
temperatureAvStereo <- projectRaster(temperatureAv, crs = myCrs)

temperatureDf <- as.data.frame(temperatureAvStereo, xy=T) %>% 
  filter(!is.na(layer))

temperatureDfSample <- temperatureDf %>% 
  mutate(layerNorm = (layer + abs(min(layer)))/(abs(min(layer)) + max(layer))) %>% 
  slice(sample(1:nrow(temperatureDf), 3000, prob = layerNorm^1)) %>% 
  filter(between(x, -4e6,4e6)) %>% 
  filter(between(y, -4.5e6,4.5e6)) 

try <- deldir(temperatureDfSample$x,temperatureDfSample$y,wl='tr')

tryDf1 <- try[[1]]%>% 
  mutate(distance = sqrt((x2-x1)^2 + (y2-y1)^2)) %>% 
  filter(distance < 500001) 

valueP1 <- raster::extract(temperatureAvStereo, tibble(x = tryDf1$x1,
                                                  y = tryDf1$y1))
valueP2 <- raster::extract(temperatureAvStereo, tibble(x = tryDf1$x2,
                                                  y = tryDf1$y2))
...
pp <- lst_result2 %>% 
  ggplot(.,aes(x = x, y = y)) +
  geom_path(aes(size = value, alpha = value, group = pointNo, colour = value),
            ) +#, size = 1.3, alpha = 0.8)+
  scale_colour_gradient(low = '#9fc97f', high = '#f75252') +
  scale_size_continuous(range = c(0.1,1.8)) +
  scale_alpha_continuous(range = c(0.1,1)) +
  geom_point(data = temperatureDfSample,
...
ggsave('~/github/scrollytellJs/visuals/temperatureVoronoi.png', pp, 
